...
import com.example.gogame.GoGame.infoMessage.Move;
import com.example.gogame.GoGame.infoMessage.MoveType;
...
class MCTSNode {
    ...
    public MCTSNode UCTSelectChild() {
        ...
        for (MCTSNode node : children) {
            if (node.getMove().getType().equals(MoveType.SKIP) && selected == null) {
                selected = node;
            } else {
                //disincentivize playing on the edges
                double ratio;
                if (node.getMove().getRow() == 0 || node.getMove().getCol() == 0 ||
                        node.getMove().getRow() == go - 1 ||
                        node.getMove().getCol() == AppRunner.BOARD_SIZE - 1) {
    ...
    public Stone.StoneColor getPlayerJustMoved() {
        return playerJustMoved;
    }

    public void setPlayerJustMoved(Stone.StoneColor playerJustMoved) {
